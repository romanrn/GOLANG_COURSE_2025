...
type dumpCollection struct {
	...
	Indexes   []string         `json:"indexes"` // List of indexed field names
}
...
func NewStoreFromDump(dump []byte) (*Store, error) {
	...
	for name, collDump := range ds.Collections {
		...
		// Restore indexes
		for _, fieldName := range collDump.Indexes {
			if err := collection.CreateIndex(fieldName); err != nil {
				pkgLogger.Error("failed to create index from dump", slog.String("collection", name), slog.String("field", fieldName), slog.Any("error", err))
				return nil, fmt.Errorf("failed to create index for field '%s' in collection '%s': %w", fieldName, name, err)
			}
		}
		pkgLogger.Info("loaded collection from dump", slog.String("name", name), slog.Int("documents", len(collDump.Documents)), slog.Int("indexes", len(collDump.Indexes)))
	}
...
func (s *Store) Dump() ([]byte, error) {
	...
	for name, coll := range s.collections {
		...
		cfg := coll.cfg
		// Collect index field names
		indexNames := make([]string, 0, len(coll.indexes))
		for fieldName := range coll.indexes {
		...
		pkgLogger.Info("prepared collection for dump", slog.String("name", name), slog.Int("documents", len(docs)), slog.Int("indexes", len(indexNames)))
	}
